...
from data_process.load_data_loader import Dataset_load, Dataset_Pred
from experiments.exp_basic import Exp_Basic
from utils.tools import EarlyStopping, adjust_learning_rate, save_model, load_model
from metrics.ETTh_metrics import metric
from models.SCINet import SCINet

class Exp_load(Exp_Basic):
    def __init__(self, args):
        super(Exp_load, self).__init__(args)
    ...
    def train(self, setting):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        writer = SummaryWriter('event/run_load/{}'.format(self.args.model_name))
    ...
    def test(self, setting, evaluate=False):
        ...
        # result save
        if self.args.save:
            folder_path = 'exp/load_results/' + setting + '/'
